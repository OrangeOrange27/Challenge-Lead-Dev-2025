...
namespace Core.EntryPoint
{
    public class InitializeGameAfterAuthController : ISimpleController
    {
        ...
        public UniTask OnStart(EmptyPayloadType payload, IControllerResources resources,
            IControllerChildren controllerChildren, CancellationToken token)
        {
            return UniTask.CompletedTask;
        }

        public async UniTask<EmptyPayloadType> Execute(IControllerResources resources,
            IControllerChildren controllerChildren, CancellationToken token)
        {
