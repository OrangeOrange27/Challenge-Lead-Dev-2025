...
namespace Infra.ControllersTree.Implementations
{
	public class ControllerRunner<TPayload, TResult> : IControllerRunner<TPayload, TResult>
	{
		...
		private readonly UniTaskCompletionSource _stoppedStatusCallback = new();
		private readonly UniTaskCompletionSource _disposedStatusCallback = new();
		private List<Exception> _exceptions;

		public ControllerRunner(string parentPathInTree, IControllerWithPayloadAndReturn<TPayload, TResult> controller,
			Func<IControllerRunnerBase, IControllerChildren> controllerChildren,
			IControllerResources controllerResources)
		{
		...
		private void ThrowAggregateExceptionIfHappen()
		{
			...
			{
				var innerException = _exceptions.Count > 1
